...
noteList = [x[:-3]
            for x in os.listdir(noteDir) if re.search(r"^[0-9]+\.md$", x)]
noteList2 = [x[:-3]
             for x in os.listdir(noteDir) if re.search(r"^[mo]\.[0-9]+\.md$", x)]
noteList3 = [x[:-3]
             for x in os.listdir(noteDir) if re.search(r"^[mo]\.[0-9]+\.[0-9]+\.md$", x)]
noteList = sorted(noteList, key=lambda x: int(x))
noteList2 = sorted(noteList2, key=lambda x: (x.split('.')[0],int(x.split('.')[1])))
noteList3 = sorted(noteList3, key=lambda x: (
    int(x.split('.')[1]), int(x.split('.')[2])))

noteList = noteList+noteList2+noteList3
...
for num in noteList:
    ...
    with open(filepath, 'r') as note:
        ...
        if num in noteList3:
            title = lines[0].split(']')[0].split('.')[2][1:]
        else:
        ...
        level_color = "<span style='color:" + \
            colors[level]+"'><b>"+level+"</b></span>"
        rel_link = os.path.join("archive", num+".md")
        line = "|"+num+"|["+title+"]("+rel_link+")|" + \
            level_color+"|"+tags+"|"+mark+"|\n"
        main_lines += line
